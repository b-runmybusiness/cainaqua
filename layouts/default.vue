    ...
    <main class="relative bg-white">
      ...
      <Transition name="slide-fade">
        <section class="fixed bottom-4 right-4 rounded-2xl overflow-hidden shadow-lg bg-white z-10 w-[28vw]" v-if="openChat">
          <header class="flex justify-between p-3 bg-cyan-500">
            <div class="flex items-center justify-center gap-2">
              <Icon name="ic:round-whatsapp" class="w-8 h-8" />
              <span>WhatsApp</span>
            </div>
            <button class="bg-cyan-600/25 flex items-center justify-center rounded-full h-9 w-9" @click="openChat = !openChat">
          ...
          <footer class="px-4 flex justify-end pb-4">
            <button class="flex items-center gap-2 bg-cyan-500 rounded-xl px-4 py-2.5 group">
              <span>Open chat</span>
              <Icon name="carbon:send-alt-filled" class="h-6 w-6 group-hover:scale-125 transition duration-700 ease-in-out" />
            </button>
          </footer>
        </section>
        <button v-else class="fixed bottom-4 right-4 z-0 bg-cyan-500 rounded-full w-16 h-16 flex items-center justify-center" @click="openChat = !openChat">
